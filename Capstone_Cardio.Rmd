...
Data:
<br>A data set containing 70000 records of patient data. The raw data is downloaded from kaggle.com, which is a database website where data sets are publicly accessible for self investigation. A link can be find here: <br>[link](https://www.kaggle.com/sulianova/cardiovascular-disease-dataset)
...
<br> Smoking | Subjective | smoke | binary |
<br> Alcohol intake | Subjective | alco | binary |
<br> Physical activity | Subjective | active | binary |
...
g = ggplot(data = melted_cormat,aes(x = "", y = "", fill = value)) 
g + geom_tile()
```
